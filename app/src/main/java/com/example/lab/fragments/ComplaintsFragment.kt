...
class ComplaintsFragment : Fragment() {
    ...
    private fun loadComplaints() {
        ...
        GlobalScope.launch(Dispatchers.IO) { // replaces doAsync (runs on another thread)
            val p1= Product(1,"Platano",2200,(0).toFloat())
                productDao.insertAll(p1)
            val p2= Product(2,"Palta",4000,(0).toFloat())
                productDao.insertAll(p2)
            val p3 = Product(3,"Pera",1500,(0).toFloat())
                productDao.insertAll(p3)
            val p4 = Product(4,"Manzana",1800,(0).toFloat())
    ...
    private fun setListOnClickListener() {
        complaintsListView.setOnItemClickListener { _, _, position, _ ->
            val selectedComplaint = (complaintsListView.adapter).getItem(position) as Product
            startActivity(
